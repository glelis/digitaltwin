...
def recorrencia(df, dados, dataSheetComp, tags_suc, tags_desc):
    ...
    EOS_INFO['lista'] = df.apply(lambda x : eos_estagios(x, tags_suc, tags_desc, dados, dataSheetComp), axis = 1)
...
def eos_estagios (x, tags_suc, tags_desc, dados, dataSheetComp):
...
def consolida_value_estagio(EOS_2oSUC, EOS_2oDESC, Tsys, Psys, dataSheetComp):
    ...
    try: 
        ...
        #     print("\n")
            ...
            resultado = [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]
    except:
        resultado = [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]
    return resultado
